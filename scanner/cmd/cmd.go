...
func Init(useDesc, shortDesc, longDesc string) Config {
	...
	domainlistCmd = NewCmd("domainlist", "File based scanning mode", "File based scanning mode", "DOMAINLIST")
	...
	// domainlistCmd command help
	helpDomainlistCmd := domainlistCmd.HelpFunc()
	newDomainlistHelpCmd := func(c *cobra.Command, args []string) {
		helpFlag = true
		helpDomainlistCmd(c, args)
	}
	domainlistCmd.SetHelpFunc(newDomainlistHelpCmd)
	...
	var cfgStats *stats.Stats
	if !cfgNoStats {
		cfgStats = stats.NewStats()
	}
