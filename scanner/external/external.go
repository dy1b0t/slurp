	...
	for {
		...
		go func(pd PermutatedDomain) {
			...
			if resp.StatusCode == 200 {
				publicMsg := fmt.Sprintf("PUBLIC http://%s (http://%s.%s)", pd.Permutation, pd.Domain.Domain, pd.Domain.Suffix)
				log.Info(publicMsg)
				_, err := file.WriteString(publicMsg + "\n")
				if err != nil {
					log.Error(err)
				}
				...
				unknownMsg := fmt.Sprintf("UNKNOWN http://%s (http://%s.%s) (%d)", pd.Permutation, pd.Domain.Domain, pd.Domain.Suffix, resp.StatusCode)
				log.Info(unknownMsg)
				_, err := file.WriteString(unknownMsg + "\n")
				if err != nil {
					log.Error(err)
				}
			}
